...
The Select component is ideal when you have 4 or more options to avoid cluttering the interface with radio buttons. It supports various states (hover, focus, error, disabled), multiple sizes, flexible label positioning, and full RTL support for Arabic interfaces.

**Key Features:**
- Built with Radix UI Select primitives for accessibility
- Support for both controlled and uncontrolled modes
- Flexible label positioning (above field or inside field)
- Multiple size variants (micro, medium, large)
- Full RTL support with proper text rendering
...
The Select component supports two label positions: above the field (default) or inside the field.
...
<Canvas of={SelectStories.InsideLabel} />
...
**When to use inside labels:**
- When vertical space is limited
- For cleaner, more modern interfaces
- When you want to reduce visual clutter

**When to use above labels:**
- When clarity is most important
- For better accessibility (always visible)
- When label text is long
...
<Canvas of={SelectStories.SizeMicro} />
...
<Canvas of={SelectStories.SizeMedium} />
...
<Canvas of={SelectStories.SizeLarge} />
...
<Canvas of={SelectStories.WithPrefix} />
...
<Canvas of={SelectStories.LongOptionList} />
...
<Canvas of={SelectStories.RTLExample} />
...
<Canvas of={SelectStories.RTLInsideLabel} />
...
<Canvas of={SelectStories.RTLWithError} />
...
- Label positioning works correctly in both directions
...
```jsx
import { selectSizes, selectLabelPositions } from '@tagaddod-design/react';

// Available sizes: ['micro', 'medium', 'large']
console.log(selectSizes);

// Available label positions: ['above', 'inside']
console.log(selectLabelPositions);
```
