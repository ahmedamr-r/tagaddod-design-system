...
// Load from source files (before build)
export async function loadTokensFromSource(): Promise<TokenSet> {
  try {
    // Import source JSON files directly
    const [primitives, semantics] = await Promise.all([
      import('@tagaddod/tokens/src/primitives.json'),
      import('@tagaddod/tokens/src/semantics.json'),
    ]);
    ...
    try {
      const [tagaddod, greenpan] = await Promise.all([
        import('@tagaddod/themes/src/tagaddod.json'),
        import('@tagaddod/themes/src/greenpan.json'),
      ]);
    ...
    console.error('Error loading tokens from source:', error);
...
// Load from built files (after build)
